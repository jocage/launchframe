...
Copy-paste this into your AI coding agent (Cursor, Claude Code, Copilot, Codex) to bootstrap context for a generated project:

```
I'm working on a SaaS app generated by Launchframe (create-launchframe).

Stack: Next.js 16 App Router, React 19, TypeScript 5.9 strict, Drizzle ORM + PostgreSQL, Better Auth (email/password + optional GitHub OAuth), Stripe/Polar billing abstraction, Resend email, Vitest + Playwright, ESLint 9 flat config.

Key files:
- AGENTS.md — full rules, commands, patterns, MUST DO / MUST NOT DO
- ARCHITECTURE.md — directory tree, data flow, extension points
- src/lib/env.ts — Zod-validated environment schema
- src/db/schema/ — Drizzle schema (auth tables + project tables)
- src/lib/billing/ — provider abstraction (Stripe/Polar behind one interface)
- src/lib/auth.ts — Better Auth server config
- src/app/ — App Router pages (thin route files, logic in lib/)

Rules: server components by default, "use client" only when needed, no barrel exports, keep route files thin, all env vars through lib/env.ts, Drizzle for all DB access.

Read AGENTS.md first for the complete reference.
```
